...
class TestSphereIntersect(unittest.TestCase):
    ...
    def confirm_hit(self, sphere, ray):
        ...
        np.testing.assert_almost_equal(
            ray.origin + hit.t * ray.direction, hit.point)
        np.testing.assert_almost_equal(
            normalize(hit.point - sphere.center), hit.normal)
        self.assertAlmostEqual(np.linalg.norm(
            hit.point - sphere.center), sphere.radius)
    ...
    def test_unitsphere_hits(self):
        ...
        # dead center hit
        hit = self.confirm_hit(unit_sphere, Ray(
            vec([2.0, 0.0, 0.0]), vec([-1.0, 0.0, 0.0])))
        self.assertAlmostEqual(hit.t, 1.0)
        # dead center with non-unit direction
        hit = self.confirm_hit(unit_sphere, Ray(
            vec([3.0, 0.0, 0.0]), vec([-2.0, 0.0, 0.0])))
        self.assertAlmostEqual(hit.t, 1.0)
        # off center hit
        hit = self.confirm_hit(unit_sphere, Ray(
            vec([1.0, 0.5, 0.0]), vec([-1.0, 0.0, 0.0])))
        ...
        hit = self.confirm_hit(unit_sphere, Ray(
            vec([2.0, 3.0, 5.0]), vec([-2.0, -3.0, -4.0])))
        self.assertAlmostEqual(hit.t, 1.0)
        # dead center hit, start inside
        hit = self.confirm_hit(unit_sphere, Ray(
            vec([2.0, 0.0, 0.0]), vec([-1.0, 0.0, 0.0]), 1.1))
    ...
    def test_nonunit_hits(self):
        ...
        hit = self.confirm_hit(sphere, Ray(
            vec([5.0, -5.0, -7.0]), vec([-3.0, 0.0, 0.0])))
        self.assertAlmostEqual(hit.t, 1.0)
        hit = self.confirm_hit(sphere, Ray(
            vec([8.0, -5.0, -7.0]), vec([-6.0, 0.0, 0.0])))
        self.assertAlmostEqual(hit.t, 1.0)
        hit = self.confirm_hit(sphere, Ray(
            vec([2.0, -3.5, -7.0]), vec([-3.0, 0.0, 0.0])))
        self.assertAlmostEqual(hit.t, 1 - np.sin(np.pi/3))
        hit = self.confirm_hit(sphere, Ray(
            vec([5.0, 4.0, 5.0]), vec([-2.0, -3.0, -4.0])))
        self.assertAlmostEqual(hit.t, 3 - 3 / np.sqrt(29))
        hit = self.confirm_hit(sphere, Ray(
            vec([5.0, 4.0, 8.0]), vec([-2.0, -3.0, -4.0])))
        self.assertAlmostEqual(hit.t, 3.0)
...
class TestCamera(unittest.TestCase):
    ...
    def test_default_camera(self):
        ...
        # print("DIRECTION: ", ray.origin, ray.direction)
        ...
        assert_direction_matches(
            dirab, 0.2 * dir00 + 0.2 * dir10 + 0.6 * dir01)
    ...
    def test_square_frame(self):
        ...
        cam = Camera(eye=vec([1, 2, 2]), target=vec(
            [1, 4, 2]), up=vec([0, 0, 1]))
